...
Public Class InvKBA_ExciseReport
    ...
    Private Sub FillGroupdetails()
        ...
        sqlstring = "SELECT  distinct (ISNULL(I.groupcode,'') )AS groupcode,ISNULL(g.groupdesc,'') AS groupdesc FROM INV_INVENTORYITEMMASTER AS I  inner join inventorygroupmaster g on i.groupcode=g.Groupcode  WHERE isnull(VOID,'') <> 'Y' ORDER BY groupcode"
    ...
    Private Sub VIEWSTOCKSUMMARY()
        ...
        SQL1 = "UPDATE Inv_ExiseRpt SET priority=0 WHERE groupdesc NOT  in ('WINE','BEER')"
        gconnection.ExcuteStoreProcedure(SQL1)
        SQL1 = "UPDATE Inv_ExiseRpt SET priority=1 WHERE groupdesc='TOTAL'"
        gconnection.ExcuteStoreProcedure(SQL1)
        SQL1 = "UPDATE Inv_ExiseRpt SET priority=2 WHERE groupdesc  in ('WINE','BEER')"
        gconnection.ExcuteStoreProcedure(SQL1)

        sqlstring = " select * from Inv_ExiseRpt order by PRIORITY"
        Me.Cursor = Cursors.WaitCursor
