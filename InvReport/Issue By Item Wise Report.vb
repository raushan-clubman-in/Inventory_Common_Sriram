...
Public Class Frm_IssueByItemwise
    Dim gconnection As New GlobalClass
    Dim sqlstring As String

    Private Sub Frm_IssueByItemwise_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Frm_IssueByItemwise_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub VIEWISSUEREGISTER()
        Try
            ...
            If Mid(UCase(gShortname), 1, 4) = "RMV" Or gShortname = "SKYYE" Then
                r = New Crystalissue_GroupwiseRMV
            Else
                r = New Crystalissue
            End If
            ...
            If UCase(gShortname) = "RMV" Or gShortname = "SKYYE" Then
                sqlstring = " SELECT Itemcode, Itemname,uom,sum(qty) as qty,sum(AMOUNT) as amount,opstorelocationcode,opstorelocationname,storelocationcode,storelocationname,categorycode FROM TEMPSTOCKISSUEDETAIL "
            Else
                sqlstring = " SELECT Itemcode, Itemname,uom,sum(qty) as qty,sum(AMOUNT) as amount,opstorelocationcode,opstorelocationname,storelocationcode,storelocationname FROM TEMPSTOCKISSUEDETAIL "
            End If
            ...
            If UCase(gShortname) = "RMV" Or gShortname = "SKYYE" Then
                sqlstring = sqlstring & " AND STORELOCATIONCODE = '" & Trim(txt_mainstorecode.Text) & "'  group by Itemcode, Itemname,uom,opstorelocationcode,opstorelocationname,storelocationcode,storelocationname,categorycode "
            Else
                sqlstring = sqlstring & " AND STORELOCATIONCODE = '" & Trim(txt_mainstorecode.Text) & "'  group by Itemcode, Itemname,uom,opstorelocationcode,opstorelocationname,storelocationcode,storelocationname "
            End If
            If UCase(gShortname) = "JIC" Or Mid(UCase(Trim(gCompanyname)), 1, 3) = "JUB" Then
    ...
    Private Sub ButExport_Click(sender As Object, e As EventArgs) Handles ButExport.Click
        Try
            ...
            sqlstring = "exec cp_stockissue"
            gconnection.ExcuteStoreProcedure(sqlstring)
            sqlstring = " SELECT 	storelocationname,	opstorelocationname,	Itemcode,	Itemname,	UoM,	qty,		rate,	AMOUNT FROM TEMPSTOCKISSUEDETAIL "
            If CheckedListBox1.CheckedItems.Count <> 0 Then
            ...
            End If
            If CheckedListBox3.CheckedItems.Count <> 0 Then
                sqlstring = sqlstring & " and isnull(void,'N')<>'Y' AND ITEMCODE IN ("
                For i = 0 To CheckedListBox3.CheckedItems.Count - 1
                    ITEMcode = Split(CheckedListBox3.CheckedItems(i), "-->")
                    sqlstring = sqlstring & "'" & ITEMcode(0) & "', "
                Next
            ...
            sqlstring = sqlstring & " '" & Format(dtpfromdate.Value, "dd/MMM/yyyy") & "' AND ' " & Format(dtptodate.Value, "dd/MMM/yyyy") & "'"
            sqlstring = sqlstring & " AND STORELOCATIONCODE = '" & Trim(txt_mainstorecode.Text) & "' order by opstorelocationname "
