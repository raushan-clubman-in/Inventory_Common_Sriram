...
Public Class InvIssueRegister
    ...
    Private Sub FillGroupdetails()
        ...
        sqlstring = "SELECT ISNULL(Groupcode,'') +' -> '+ ISNULL(Groupdesc,'') AS Groupdesc FROM InventoryGroupMaster  where Groupcode in (select groupcode from INV_INVENTORYITEMMASTER) and isnull(freeze,'') <> 'Y' ORDER BY Groupcode "
    ...
    Private Sub InvIssueRegister_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If UCase(gShortname) = "BGC" Then
            ENTRY_ISSUE.Visible = True
        Else
            ENTRY_ISSUE.Visible = False
        End If
    ...
    Private Sub VIEWISSUEREGISTERsUMMARY()
        Try
            ...
            If gShortname = "SKYYE" Then
                r = New CryIssueSUMM_SKYYE
            Else
                r = New CryIssueReportSummary
            End If
            ...
            If gShortname = "SKYYE" Then
                ...
                sqlstring = "  Select  OPSTORELOCATIONNAME,SUM(AMOUNT)AS AMOUNT,CATEGORYCODE FROM TEMPSTOCKISSUEDETAIL"
            ...
            If gShortname = "SKYYE" Then
                sqlstring = sqlstring & " GROUP BY OPSTORELOCATIONNAME,CATEGORYCODE"
            End If
            'If UCase(gShortname) = "JIC" Or Mid(UCase(Trim(gCompanyname)), 1, 3) = "JUB" Then
            If gShortname <> "SKYYE" Then
    ...
    Private Sub Cmd_View_Click(sender As Object, e As EventArgs) Handles Cmd_View.Click
        ...
        If chksummary.Checked = True Then
            VIEWISSUEREGISTERsUMMARY()
        ElseIf CK_export.Checked = True Then
            Excel_export()
        Else
    ...
    Private Sub VIEWISSUEREGISTER()
        Try
            ...
            Dim R
            If gShortname = "SKYYE" Then
                R = New CryIssueReport_SKYEE
            Else
                R = New CryIssueReport
            End If
            ...
            If Mid(UCase(gShortname), 1, 3) = "BGC" Then
                ...
                Else
            End If
            If ENTRY_ISSUE.Checked = True Then
                sqlstring = "SELECT * FROM STOCKISSUEDETAIL"
                sqlstring = sqlstring & " WHERE CAST(CONVERT(VARCHAR(11),DOCDATE ,106) AS DATETIME) BETWEEN"
                sqlstring = sqlstring & "'" & Format(dtpfromdate.Value, "dd/MMM/yyyy") & "' AND '" & Format(dtptodate.Value, "dd/MMM/yyyy") & "'"
                If CheckedListBox1.CheckedItems.Count <> 0 Then
                    sqlstring = sqlstring & " AND  isnull(VOID,'N')<> 'Y' and opstorelocationCODE IN ("
                    For i = 0 To CheckedListBox1.CheckedItems.Count - 1
                        STORECODE = Split(CheckedListBox1.CheckedItems(i), "-")
                        sqlstring = sqlstring & " '" & Trim(STORECODE(0)) & "', "
                    Next
    ...
    Private Sub CK_export_CheckedChanged(sender As Object, e As EventArgs) Handles CK_export.CheckedChanged
