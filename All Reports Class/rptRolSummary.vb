...
Public Class rptRolSummary
            ...
            If gdataset.Tables("STOCKSUMMARYREPORT").Rows.Count > 0 Then
                ...
                For Each dr In gdataset.Tables("STOCKSUMMARYREPORT").Rows
                    ...
                    If SUBGROUPNAME <> Trim(CStr(dr("SUBGROUPNAME"))) Then
                        ...
                        Filewrite.Write(Space(35) & " TOTAL =======>  " & Space(13))
                        ...
                        Filewrite.WriteLine(Space(10 - Len(Mid(Format(closeval, "0.000"), 1, 10))) & Mid(Format(closeval, "0.000"), 1, 10))
                        'Filewrite.WriteLine(Space(8) & Space(12 - Len(Mid(Format(groupvalue, "0.000"), 1, 12))) & Mid(Format(groupvalue, "0.000"), 1, 12))
                        'Filewrite.WriteLine(Space(8) & Space(12 - Len(Mid(Format(GRPVAL, "0.000"), 1, 12))) & Mid(Format(GRPVAL, "0.000"), 1, 12))
                        Topval = Topval + opval
                    ...
                    Filewrite.Write("{0,10}", Mid(Format(Val(dr("ROL")), "0"), 1, 10))
                    ...
                    Filewrite.WriteLine("{0,19}", Mid(Format(Val(dr("CLSQTY")), "0.000"), 1, 10))
                    ...
                    'SALEVAL = Val(dr("SALERATE")) * Val(dr("CLSQTY"))
                ...
                Filewrite.Write(Space(39) & " GRAND TOTAL =======>   " & Space(2))
                ...
                Filewrite.WriteLine(Space(10 - Len(Mid(Format(Tcloseval, "0.000"), 1, 10))) & Mid(Format(Tcloseval, "0.000"), 1, 10))
                'Filewrite.WriteLine(Space(8) & Space(12 - Len(Mid(Format(Tgroupvalue, "0.000"), 1, 12))) & Mid(Format(Tgroupvalue, "0.000"), 1, 12))
                'Filewrite.WriteLine(Space(8) & Space(12 - Len(Mid(Format(TGRPVAL, "0.000"), 1, 12))) & Mid(Format(TGRPVAL, "0.000"), 1, 12))
                pagesize = pagesize + 2
                Filewrite.WriteLine(StrDup(75, "="))
                pagesize = pagesize + 1
            Else
    ...
    Private Function PrintHeader(ByVal Heading() As String, ByVal mskfromdate As Date, ByVal msktodate As Date)
        ...
        Try
            ...
            Filewrite.WriteLine(StrDup(75, "-"))
            pagesize = pagesize + 1
            Filewrite.WriteLine("ITEM   ITEM                        UOM         REORDER LEVEL     CLOSING    ")
            pagesize = pagesize + 1
            Filewrite.WriteLine("CODE   DESCRIPTION                 QTY             QTY             QTY      ")
            pagesize = pagesize + 1
            Filewrite.WriteLine(StrDup(75, "-"))
            pagesize = pagesize + 1
        Catch ex As Exception
