...
Public Class rptStockPurchaseregister
    ...
    Public Function Reportdetails(ByVal SQLSTRING As String, ByVal PAGEHEAD() As String, ByVal FROMDATE As Date, ByVal TODATE As Date)
        ...
        Try
            ...
            If gdataset.Tables("PURCHASEREGISTERREPORT").Rows.Count > 0 Then
                For Each dr In gdataset.Tables("PURCHASEREGISTERREPORT").Rows
                    ...
                    If SUPPLIERNAME <> Trim(CStr(dr("SUPPLIERNAME"))) Then
                        If SUPPLIERNAMEBOOL = True Then
                            Filewrite.WriteLine()
                            pagesize = pagesize + 1
                            Filewrite.WriteLine(StrDup(157, "*"))
                            pagesize = pagesize + 1

                            If Not PAGEHEAD Is "GRN CUM PURCHASE BILL" Then
                                ...
                                ' Filewrite.WriteLine(StrDup(157, "*"))
                                pagesize = pagesize + 1
                                Filewrite.WriteLine("{0,-10},{1,-40}", Chr(14) & Chr(15) & Mid(Trim(CStr(dr("SUPPLIERCODE"))), 1, 10), Mid(Trim(CStr(dr("SUPPLIERNAME"))), 1, 40))
                                pagesize = pagesize + 1
                        ...
                        If gdataset.Tables("VIEWPURCHASEREGISTERSUMMARY").Rows.Count > 0 Then
                            For I = 0 To gdataset.Tables("VIEWPURCHASEREGISTERSUMMARY").Rows.Count - 1 Step 1
                                With gdataset.Tables("VIEWPURCHASEREGISTERSUMMARY").Rows(I)
                                    ...
                                    End If
                                    Filewrite.Write("{0,-10}", Mid(Trim(Val(.Item("SUPPLIERINVNO"))), 1, 10))
                                    ...
                                    Filewrite.Write("{0,11}", Mid(Format(Val(.Item("TAXAMOUNT")), "0.00"), 1, 12))
                            ...
                            Filewrite.WriteLine(StrDup(157, "."))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine("{0,-20}{1,-35}{2,30}{3,-20}{4,12}{5,15}", "PAID DETAILS", AVOUCHERDETAILS, "BASIC :", "", "", Format(Val(dblBasicamount), "0.00"))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine("{0,-20}{1,-35}{2,30}{3,-20}{4,12}{5,15}", "************", "", "V.A.T :", "", "", Format(Val(dblVATamount), "0.00"))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine("{0,-20}{1,-35}{2,30}{3,-20}{4,12}{5,15}", "", "", "OTHER CHARGES :", "", "", Format(Val(dblothercharge), "0.00"))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine("{0,-20}{1,-35}{2,30}{3,-20}{4,12}{5,15}", "", "", "DISCOUNT AMOUNT :", "", "", Format(Val(dbldiscountamt), "0.00"))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine("{0,-20}{1,-35}{2,30}{3,-20}{4,12}{5,15}", "", "", "BILL AMOUNT :", "", "", Format(Val(dblBillamount), "0.00"))
                            pagesize = pagesize + 1
                            Filewrite.WriteLine(StrDup(157, "."))
                            pagesize = pagesize + 1
                ...
                'Filewrite.WriteLine(StrDup(157, "*"))
                pagesize = pagesize + 1
                If Not PAGEHEAD(0) Is "GRN CUM PURCHASE BILL" Then
                    ' Filewrite.Write("{0,-20}{1,-15} {2,-10}{3,-40}{4,-10}{5,-10}", "GROUP TOTAL =====>", "", "", "", Mid(Format(Val(dblGroupBasicamt), "0.00"), 1, 10), Mid(Format(Val(dblGroupVATamt), "0.00"), 1, 10))
                    'Filewrite.WriteLine("{0,12}{1,15}{2,13}", Mid(Format(Val(dblGroupOtherchg), "0.00"), 1, 12), Mid(Format(Val(dblGroupdiscountamt), "0.00"), 1, 15), Mid(Format(Val(dblGroupbillamt), "0.00"), 1, 13))
                    pagesize = pagesize + 1
                    ' Filewrite.WriteLine(StrDup(157, "*"))
                    pagesize = pagesize + 1
                    '  Filewrite.WriteLine(StrDup(157, "="))
                    pagesize = pagesize + 1
                    'Filewrite.Write("{0,-20}{1,-15} {2,-10}{3,-40}{4,-10}{5,-10}", "GRAND TOTAL =====>", "", "", "", Mid(Format(Val(dblGrandBasicamt), "0.00"), 1, 10), Mid(Format(Val(dblGrandVATamt), "0.00"), 1, 10))
    ...
    Private Function PrintHeader(ByVal Heading() As String, ByVal mskfromdate As Date, ByVal msktodate As Date)
        ...
        Try
            ...
            Filewrite.WriteLine(StrDup(157, "-"))
            pagesize = pagesize + 1
            Filewrite.Write("{0,-20}{1,-15} {2,-10}{3,-40}{4,-10}{5,-10}", "GRN NO.", "GRN", "BILL NO", "GRN DETAILS", "UOM", "QUANTITY")
            Filewrite.WriteLine("{0,12}{1,15}{2,13}{3,10}", "RATE", "AMOUNT", "VAT", "DISCOUNT")
            pagesize = pagesize + 1
            Filewrite.Write("{0,-20}{1,-15} {2,-10}{3,-40}{4,-10}{5,-10}", "", "DATE", "", "", "", "")
            Filewrite.WriteLine("{0,12}{1,15}{2,13}{3,10}", "", "", "AMOUNT", "AMOUNT")
            pagesize = pagesize + 1
            Filewrite.WriteLine(StrDup(157, "-"))
            pagesize = pagesize + 1
        Catch ex As Exception
