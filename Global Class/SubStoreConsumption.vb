...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Cmd_UPDATE.Location = New System.Drawing.Point(62, 8)
        ...
        Me.cmd_exit.Location = New System.Drawing.Point(343, 8)
        ...
        'PBAR
        '
        Me.PBAR.Location = New System.Drawing.Point(386, 352)
        Me.PBAR.Name = "PBAR"
        ...
        Me.Panel1.Location = New System.Drawing.Point(362, 400)
        ...
        Me.btn_validation.Location = New System.Drawing.Point(208, 7)
        ...
        Me.GroupBox1.Location = New System.Drawing.Point(362, 184)
        ...
        Me.Label4.Location = New System.Drawing.Point(362, 131)
        Me.Label4.Name = "Label4"
        Me.Label4.Size = New System.Drawing.Size(488, 23)
        Me.Label4.TabIndex = 502
        Me.Label4.Text = "MANUAL STOCK UPDATION- POS"
...
#End Region
    ...
    Private Sub Cmd_UPDATE_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_UPDATE.Click
        ...
        Dim P_KOTDATE As Date
        Dim I, J, K As Integer

        SQLSTRING = "select * from businessdate where bdate between '" & Format(dtp_FROMdate.Value, "dd-MMM-yyyy") & "' AND '" & Format(dtp_TOdate.Value, "dd-MMM-yyyy") & "'"
        gconnection.getDataSet(SQLSTRING, "bdate")
        If gdataset.Tables("bdate").Rows.Count > 0 Then
            MsgBox(" Selected Date Should not be less than Day close Date", MsgBoxStyle.Critical)
            Exit Sub
        End If
        ...
        SQLSTRING = "SELECT POSCODE, K.KOTNO,K.KOTDETAILS,K.KOTDATE,K.ITEMCODE,K.ITEMDESC,K.UOM,K.QTY,K.RATE,K.AMOUNT,K.DELFLAG," &
        " K.ADDUSERID,K.ADDDATETIME,K.UPDUSERID,K.UPDDATETIME FROM KOT_DET K INNER JOIN POSITEMSTORELINK L ON K.POSCODE=L.POS AND K.ITEMCODE=L.ITEMCODE WHERE " &
        " ISNULL(KOTSTATUS,'N')='N' AND ISNULL(DELFLAG,'')<>'Y' AND L.STORECODE='" & Trim(txt_POS.Text) & "'" &
        " AND CAST(CONVERT(VARCHAR(11),KOTDATE,6) AS DATETIME) BETWEEN '" & Format(dtp_FROMdate.Value, "dd-MMM-yyyy") & "' AND '" & Format(dtp_TOdate.Value, "dd-MMM-yyyy") & "' AND ISNULL(K.UOM,'') <> ''  ORDER BY k.ITEMCODE"
        ...
        If gdataset.Tables.Contains("KOTDETAILS") Then
            ...
            If gdataset.Tables("KOTDETAILS").Rows.Count > 0 Then
                ...
                For I = 0 To gdataset.Tables("KOTDETAILS").Rows.Count - 1
                    ...
                    P_KOTDATE = Format(CDate(Trim(gdataset.Tables("KOTDETAILS").Rows(I).Item("KOTDATE"))), "dd-MMM-yyyy")
                    'If gShortname = "KGA" Or gShortname = "HGA" Then
                    SQLSTRING = "SELECT GITEMCODE,GITEMNAME,GUOM,GQTY,GRATE,GAMOUNT,isnull(GDBLAMT,0) as GDBLAMT,isnull(GHIGHRATIO,0) as GHIGHRATIO,GGROUPCODE,GSUBGROUPCODE,VOID FROM BOM_DET WHERE"
                    SQLSTRING = SQLSTRING & " ITEMCODE='" & POSITEMCODE & "' AND ITEMUOM='" & POSITEMUOM & "' AND ISNULL(VOID,'') <> 'Y' AND  '" & Format(CDate(P_KOTDATE), "dd-MMM-yyyy") & "' BETWEEN ISNULL(StartDate,'') AND ISNULL(EndingDate,'" & Format(CDate(P_KOTDATE), "dd-MMM-yyyy") & "')  GROUP BY GITEMCODE,GITEMNAME,GUOM,GQTY,GRATE,GAMOUNT,GDBLAMT,GHIGHRATIO,GGROUPCODE,GSUBGROUPCODE,VOID"
                    ...
                    If gdataset.Tables("BOM").Rows.Count > 0 Then
                        For K = 0 To gdataset.Tables("BOM").Rows.Count - 1
                            If Trim(gdataset.Tables("KOTDETAILS").Rows(I).Item("KOTNO")) = "DIN/012025/16-17" Then
                                MsgBox("")
                            ...
                            SQLSTRING = SQLSTRING & " '" & Trim(txt_POS.Text) & "',"
                            SQLSTRING = SQLSTRING & " '" & Trim(STORELOCATION(Trim(txt_POS.Text))) & "',"
                            ...
                            AVGRATE = Trim(gdataset.Tables("KOTDETAILS").Rows(I).Item("AMOUNT")) / (dblCalqty * CDbl(gdataset.Tables("BOM").Rows(K).Item("GQTY")))
                            SQLSTRING = SQLSTRING & " '" & AVGRATE & "' ,"
                            SQLSTRING = SQLSTRING & Trim(gdataset.Tables("KOTDETAILS").Rows(I).Item("AMOUNT")) & ","
                    ...
                    Else
                        ...
                        SQLSTRING = SQLSTRING & " 1 AS HIGHRATIO, ISNULL(I.GROUPCODE,'') AS GROUPCODE, "
                        SQLSTRING = SQLSTRING & " ISNULL(I.SUBGROUPCODE,'') AS SUBGROUPCODE FROM iNV_INVENTORYITEMMASTER AS I "
                        SQLSTRING = SQLSTRING & " WHERE I.ITEMCODE='" & POSITEMCODE & "' AND I.STOCKUOM='" & POSITEMUOM & "' AND ISNULL(VOID,'') <> 'Y'"
                        ...
                        If gdataset.Tables("DIRECT_STOCK").Rows.Count > 0 Then
                            ...
                            AVGRATE = Trim(gdataset.Tables("KOTDETAILS").Rows(I).Item("AMOUNT")) / (dblCalqty)
                            SQLSTRING = SQLSTRING & " '" & AVGRATE & "',"
                            SQLSTRING = SQLSTRING & Trim(gdataset.Tables("KOTDETAILS").Rows(I).Item("AMOUNT")) & ","
                            'sqlstring = sqlstring & dblCalqty * CDbl(gdataset.Tables("BOM").Rows(K).Item("GAMOUNT")) & ","
